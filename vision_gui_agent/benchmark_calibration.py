...
from .benchmark_agent import BenchmarkTaskPolicy, PixelBenchmarkGrounder
from .visual_function_lab import LAYOUTS, TASKS
                ...
                for layout in layouts:
                    for task in tasks:
                        ...
                        result = await Agent(BenchmarkTaskPolicy(task), AgentConfig(
                            ...
                            max_steps=len(task.actions) + 2, memory_mode="none",
                        ), PixelBenchmarkGrounder()).run(page, task.goal)
                        observed = [item["action"] for item in evaluator.trace]
                        passed = result.completed and observed == list(task.actions) and all(item["effective"] for item in evaluator.trace)
                        results.append({"task": task.id, "layout": layout, "passed": passed, "run_id": result.run_id,
                                        "actions": observed, "error": result.error})
            finally:
                await browser.close()
    finally:
        server.shutdown(); server.server_close()
    return {"passed": all(item["passed"] for item in results), "runs": len(results), "results": results}


def main() -> None:
    parser = argparse.ArgumentParser(description="Calibrate the actual screenshot/input agent against Visual Function Lab")
    parser.add_argument("--artifacts", default="artifacts/benchmark-calibration")
    ...
    else: print(f"Visual Function Lab agent calibration: {'PASS' if report['passed'] else 'FAIL'} — {report['runs']} browser-agent runs")
